...
LIntExport void GetErrorText(const HTuple& ErrorNumber, HTuple* ErrorText);
...
LIntExport void BinocularDisparityMg(const HObject& Image1, const HObject& Image2, HObject* Disparity, HObject* Score, const HTuple& GrayConstancy, const HTuple& GradientConstancy, const HTuple& Smoothness, const HTuple& InitialGuess, const HTuple& CalculateScore, const HTuple& MGParamName, const HTuple& MGParamValue);
...
LIntExport void GetTextResult(const HTuple& TextResultID, const HTuple& ResultName, HTuple* Value);
...
LIntExport void GetTextModelParam(const HTuple& TextModel, const HTuple& Param, HTuple* Value);

// Set parameters of a text model.
LIntExport void SetTextModelParam(const HTuple& TextModel, const HTuple& Param, const HTuple& Value);
...
// Sets AOP information for operators.
...
// returns AOP information for operators. 
...
// queries indexing struxture of AOP information for operators. 
...
// Writes knowledge about hardware dependent behaviour of automatic operator parallelization to file.
LIntExport void WriteAopKnowledge(const HTuple& FileName, const HTuple& ParamName, const HTuple& ParamValue);

// Loads knowledge about hardware dependent behaviour of automatic operator parallelization.
...
LIntExport void TransformMetrologyObject(const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& Row, const HTuple& Column, const HTuple& Phi, const HTuple& Mode);
...
LIntExport void CopyMetrologyModel(const HTuple& MetrologyHandle, const HTuple& Indices, HTuple* CopiedMetrologyHandle);

// Copy metrology metrology objects of a metrology model.
LIntExport void CopyMetrologyObject(const HTuple& MetrologyHandle, const HTuple& Indices, HTuple* CopiedIndices);

// Get the number of instances of the metrology objects of a metrology model.
LIntExport void GetMetrologyObjectNumInstances(const HTuple& MetrologyHandle, const HTuple& Indices, HTuple* NumInstances);

// Get the results of the measurements applied for a metrology model.
LIntExport void GetMetrologyObjectResult(const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& Instance, const HTuple& GenParamName, const HTuple& GenParamValue, HTuple* Parameter);

// Get the measure regions and the results of the edge location for the metrology objects of a metrology model.
LIntExport void GetMetrologyObjectMeasures(HObject* Contour, const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& Transition, HTuple* Row, HTuple* Column);
...
LIntExport void ResetMetrologyObjectFuzzyParam(const HTuple& MetrologyHandle, const HTuple& Indices);

// Reset all parameters of a metrology model.
LIntExport void ResetMetrologyObjectParam(const HTuple& MetrologyHandle, const HTuple& Indices);

// Get a fuzzy parameter of a metroloy model.
LIntExport void GetMetrologyObjectFuzzyParam(const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& GenParamName, HTuple* GenParamValue);

// Get one or several parameters of a metroloy model.
LIntExport void GetMetrologyObjectParam(const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& GenParamName, HTuple* GenParamValue);

// Set fuzzy parameters or fuzzy functions for a metrology model.
LIntExport void SetMetrologyObjectFuzzyParam(const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& GenParamName, const HTuple& GenParamValue);

// Set parameters for the metrology objects of a metrology model.
LIntExport void SetMetrologyObjectParam(const HTuple& MetrologyHandle, const HTuple& Indices, const HTuple& GenParamName, const HTuple& GenParamValue);
...
LIntExport void ClearMetrologyObject(const HTuple& MetrologyHandle, const HTuple& Indices);
...
// Compute the generalized eigenvalues and optinally generalized eigenvectors of symmetric input matrices.
...
// Compute the eigenvalues and optinally eigenvectors of a symmetric matrix.
...
LIntExport void SetObjectModel3dAttribMod(const HTuple& ObjectModel3D, const HTuple& Name, const HTuple& Type, const HTuple& Data);

// Set attributes of a 3D object model.
LIntExport void SetObjectModel3dAttrib(const HTuple& ObjectModel3D, const HTuple& Name, const HTuple& Type, const HTuple& Data, HTuple* ObjectModel3DOut);
...
LIntExport void GetOperatorName(const HTuple& Pattern, HTuple* ProcNames);

// Get default data type for the control parameters of a HALCON-operator.
LIntExport void GetParamTypes(const HTuple& ProcName, HTuple* InpCtrlParType, HTuple* OutpCtrlParType);

// Get number of the different parameter classes of a HALCON-operator.
LIntExport void GetParamNum(const HTuple& ProcName, HTuple* CName, HTuple* InpObjPar, HTuple* OutpObjPar, HTuple* InpCtrlPar, HTuple* OutpCtrlPar, HTuple* Type);

// Get the names of the parameters of a HALCON-operator.
LIntExport void GetParamNames(const HTuple& ProcName, HTuple* InpObjPar, HTuple* OutpObjPar, HTuple* InpCtrlPar, HTuple* OutpCtrlPar);

// Get information concerning a HALCON-operator.
LIntExport void GetOperatorInfo(const HTuple& ProcName, const HTuple& Slot, HTuple* Information);

// Get information concerning the operator parameters.
LIntExport void GetParamInfo(const HTuple& ProcName, const HTuple& ParamName, const HTuple& Slot, HTuple* Information);

// Search names of all operators assigned to one keyword.
LIntExport void SearchOperator(const HTuple& Keyword, HTuple* ProcNames);

// Get keywords which are assigned to operators.
LIntExport void GetKeywords(const HTuple& ProcName, HTuple* Keywords);
...
LIntExport void OpticalFlowMg(const HObject& Image1, const HObject& Image2, HObject* VectorField, const HTuple& Algorithm, const HTuple& SmoothingSigma, const HTuple& IntegrationSigma, const HTuple& FlowSmoothness, const HTuple& GradientConstancy, const HTuple& MGParamName, const HTuple& MGParamValue);
...
LIntExport void GenImageSurfaceSecondOrder(HObject* ImageSurface, const HTuple& Type, const HTuple& Alpha, const HTuple& Beta, const HTuple& Gamma, const HTuple& Delta, const HTuple& Epsilon, const HTuple& Zeta, const HTuple& Row, const HTuple& Col, const HTuple& Width, const HTuple& Height);

// Create a tilted gray surface with first order polynomial.
LIntExport void GenImageSurfaceFirstOrder(HObject* ImageSurface, const HTuple& Type, const HTuple& Alpha, const HTuple& Beta, const HTuple& Gamma, const HTuple& Row, const HTuple& Col, const HTuple& Width, const HTuple& Height);
...
LIntExport void CreateCaltab(const HTuple& Width, const HTuple& CalTabDescrFile, const HTuple& CalTabFile);

// Generate a calibration plate description file and a corresponding PostScript file.
LIntExport void GenCaltab(const HTuple& XNum, const HTuple& YNum, const HTuple& MarkDist, const HTuple& DiameterRatio, const HTuple& CalTabDescrFile, const HTuple& CalTabPSFile);

// Read the mark center points from the calibration plate description file.
LIntExport void CaltabPoints(const HTuple& CalTabDescrFile, HTuple* X, HTuple* Y, HTuple* Z);
...
LIntExport void SimCaltab(HObject* SimImage, const HTuple& CalTabDescrFile, const HTuple& CameraParam, const HTuple& CaltabPose, const HTuple& GrayBackground, const HTuple& GrayCaltab, const HTuple& GrayMarks, const HTuple& ScaleFac);

// Project and visualize the 3D model of the calibration plate in the image.
LIntExport void DispCaltab(const HTuple& WindowHandle, const HTuple& CalTabDescrFile, const HTuple& CameraParam, const HTuple& CaltabPose, const HTuple& ScaleFac);
...
LIntExport void FindCaltab(const HObject& Image, HObject* Caltab, const HTuple& CalTabDescrFile, const HTuple& SizeGauss, const HTuple& MarkThresh, const HTuple& MinDiamMarks);
...
// Find the standard calibration plate and set the extracted points and contours in a calibration data model.
