...
# define HGetPElemL(PH,P,C,V,N)                                         \
  do{                                                                   \
    Herror _ERR;                                                        \
    _ERR = HPGetPElemL(PH,P,C,V,N);                                     \
    if (_ERR != H_MSG_OK) return(_ERR);                                 \
    _ERR = IOSpyElem(PH,P,*(INT4_8**)(V),*(INT4_8*)(N),LONG_PAR,TRUE);  \
    if (_ERR != H_MSG_OK) return(_ERR);                                 \
  }while(0)
#else
# define HGetPElemL(PH,P,C,V,N)                 \
  HCkP( HPGetPElemL(PH,P,C,V,N) )       
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HGetPElemD(PH,P,C,V,N)                                         \
  do{                                                                   \
    Herror _ERR;                                                        \
    _ERR = HPGetPElemD(PH,P,C,V,N);                                     \
    if (_ERR != H_MSG_OK) return(_ERR);                                 \
    if (HROInfo.Spy == TRUE)                                            \
    {                                                                   \
      void const **tmp = (void*)(V);                                    \
      _ERR = IOSpyElem(PH,P,*tmp,*(INT4_8*)(N),DOUBLE_PAR,TRUE);        \
      if (_ERR != H_MSG_OK) return(_ERR);                               \
    }                                                                   \
  }while(0)
#else
# define HGetPElemD(PH,P,C,V,N)                 \
  HCkP( HPGetPElemD(PH,P,C,V,N) ) 
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HGetPElemS(PH,P,C,V,N)                                            \
  do{                                                                      \
    Herror _ERR;                                                           \
    _ERR = HPGetPElemS(PH,P,C,V,N);                                        \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
    _ERR = IOSpyElem(PH,P,*(char***)(V),*(INT4_8*)(N),STRING_PAR,TRUE);    \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
  }while(0)
#else
# define HGetPElemS(PH,P,C,V,N)                 \
  HCkP( HPGetPElemS(PH,P,C,V,N) )       
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HGetPElem(PH,P,V,N,T)                                             \
  do{                                                                      \
    Herror _ERR;                                                           \
    HPGetPElem(PH,P,V,N,T);                                                \
    _ERR = IOSpyElem(PH,P,*(void**)(V),*(INT4_8*)(N),*(INT*)(T),TRUE);     \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
  }while(0)
#else
# define HGetPElem(PH,P,V,N,T)                  \
...
#if !defined(NO_SPY) && !defined(SMALL)
# define HGetElemL(PH,P,C,M,V,N)                                           \
  do{                                                                      \
    Herror _ERR;                                                           \
    _ERR = HPGetElemL(PH,P,C,M,V,N);                                       \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
    _ERR = IOSpyElem(PH,P,*(INT4_8**)(V),*(INT4_8*)(N),LONG_PAR,TRUE);     \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
  }while(0)
#else
# define HGetElemL(PH,P,C,M,V,N)                \
  HCkP( HPGetElemL(PH,P,C,M,V,N) )       
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HGetElemD(PH,P,C,M,V,N)                                           \
  do{                                                                      \
    Herror _ERR;                                                           \
    _ERR = HPGetElemD(PH,P,C,M,V,N);                                       \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
    _ERR = IOSpyElem(PH,P,*(double**)(V),*(INT4_8*)(N),DOUBLE_PAR,TRUE);   \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
  }while(0)
#else
# define HGetElemD(PH,P,C,M,V,N)                \
  HCkP( HPGetElemD(PH,P,C,M,V,N) )       
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HGetElemS(PH,P,C,M,V,N)                                           \
  do{                                                                      \
    Herror _ERR;                                                           \
    _ERR = HPGetPElemS(PH,P,C,M,V,N);                                      \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
    _ERR = IOSpyElem(PH,P,*(char***)(V),*(INT4_8*)(N),STRING_PAR,TRUE);    \
    if (_ERR != H_MSG_OK) return(_ERR);                                    \
  }while(0)
#else
# define HGetElemS(PH,P,C,M,V,N)                \
  HCkP( HPGetElemS(PH,P,C,M,V,N) )       
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HCopyElemL(PH,P,C,V,N)                          \
  do{                                                    \
    Herror _ERR;                                         \
    _ERR = HPCopyElemL(PH,P,C,V,N);                      \
    if (_ERR != H_MSG_OK) return(_ERR);                  \
    _ERR = IOSpyElem(PH,P,V,*(INT4_8*)(N),LONG_PAR,TRUE);\
    if (_ERR != H_MSG_OK) return(_ERR);                  \
  }while(0)
#else
# define HCopyElemL(PH,P,C,V,N)                \
  HCkP( HPCopyElemL(PH,P,C,V,N) )       
#endif

#if !defined(NO_SPY) && !defined(SMALL)
# define HCopyElemD(PH,P,C,V,N)                             \
  do{                                                       \
    Herror _ERR;                                            \
    _ERR = HPCopyElemD(PH,P,C,V,N);                         \
    if (_ERR != H_MSG_OK) return(_ERR);                     \
    _ERR = IOSpyElem(PH,P,V,*(INT4_8*)(N),DOUBLE_PAR,TRUE); \
    if (_ERR != H_MSG_OK) return(_ERR);                     \
  }while(0)
#else
# define HCopyElemD(PH,P,C,V,N)               \
  HCkP( HPCopyElemD(PH,P,C,V,N) )       
#endif
